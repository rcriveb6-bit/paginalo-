...
export async function POST(req: NextRequest) {
  ...
  if (useMock) return NextResponse.json({ ok: true, id: "mock-" + Date.now() });
  const { data, error } = await supabase.from("rya_eventos").insert([body]).select().single();
...
export async function PUT(req: NextRequest) {
  ...
  const { data, error } = await supabase.from("rya_eventos").update(body).eq("id", id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: "ID required" }, { status: 400 });
  if (useMock) return NextResponse.json({ ok: true });
  const { error } = await supabase.from("rya_eventos").update({ activo: false }).eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
