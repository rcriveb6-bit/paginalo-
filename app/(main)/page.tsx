  ...
  return (
    <div style={{ background: BG, color: "#dbe2f7", fontFamily: "var(--font-dm-sans), system-ui, sans-serif" }} className="min-h-dvh overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50" style={{ background: "rgba(11,19,34,0.85)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(255,255,255,0.07)", paddingTop: "env(safe-area-inset-top)" }}>
        <div className="max-w-[1100px] mx-auto px-6 md:px-12 flex justify-between items-center h-20">
          ...
          <a href={wa} target="_blank" rel="noopener noreferrer"
            className="hidden md:flex items-center gap-2 rounded-full px-6 py-2 text-sm font-bold transition-all hover:scale-105 active:scale-95"
      ...
      <section className="py-12 trust-strip" style={{ borderTop:"1px solid rgba(255,255,255,0.07)", borderBottom:"1px solid rgba(255,255,255,0.07)", background:"rgba(255,255,255,0.02)" }}>
        <div className="max-w-[1100px] mx-auto px-6 md:px-12 flex flex-wrap justify-center md:justify-between items-center gap-8">
          {logos.map(l => (
      ...
      <section className="py-28 px-6 md:px-12 max-w-[1100px] mx-auto" id="servicios">
        <motion.div className="text-center mb-16 space-y-4"
          ...
          <p className="text-[11px] font-black tracking-[0.2em] uppercase" style={{ color:TEAL }}>Lo que hacemos</p>
          <h2 className="font-black text-balance"
            style={{ fontFamily:"var(--font-syne)", color:BONE, fontSize:"32px", lineHeight:"40px", letterSpacing:"-0.01em" }}>
            Todo lo que necesita tu negocio
          </h2>
          <p className="max-w-xl mx-auto text-pretty" style={{ color:MUTED, fontSize:"15px" }}>Tecnología de punta adaptada a la escala de tu negocio en Puerto Rico.</p>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <motion.div key={s.title}
              ...
              <h3 className="font-black mb-4 text-balance" style={{ fontFamily:"var(--font-syne)", color:BONE, fontSize:"24px", lineHeight:"32px" }}>{s.title}</h3>
              <p className="text-pretty" style={{ color:MUTED, fontSize:"15px" }}>{s.desc}</p>
            </motion.div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-[1100px] mx-auto">
          <motion.div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6"
            ...
            <div className="space-y-4">
              <p className="text-[11px] font-black tracking-[0.2em] uppercase" style={{ color:TEAL }}>Metodología</p>
              <h2 className="font-black text-balance"
                style={{ fontFamily:"var(--font-syne)", color:BONE, fontSize:"32px", lineHeight:"40px", letterSpacing:"-0.01em" }}>Del acuerdo al lanzamiento</h2>
            </div>
            <p className="max-w-sm text-pretty" style={{ color:MUTED, fontSize:"15px" }}>Un proceso ágil y transparente. Sin sorpresas, sin retrasos.</p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            ...
            {[
              ...
              <motion.div key={s.n} className="space-y-4"
                ...
                <div className="size-16 rounded-2xl glass-card-dark flex items-center justify-center font-black"
                  style={s.active ? { fontFamily:"var(--font-syne)", color:TEAL, fontSize:"24px", borderColor:`rgba(77,218,218,0.2)`, background:`rgba(77,218,218,0.05)` } : { fontFamily:"var(--font-syne)", color:BONE, fontSize:"24px" }}>
                  {s.n}
                </div>
                <h4 className="font-black" style={{ fontFamily:"var(--font-syne)", color:BONE, fontSize:"24px", lineHeight:"32px" }}>{s.title}</h4>
                <p className="text-pretty" style={{ color:MUTED, fontSize:"15px" }}>{s.desc}</p>
                <span className="text-[11px] font-black tracking-[0.2em] uppercase" style={{ color: s.active ? TEAL : MUTED }}>{s.day}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-28 px-6 md:px-12 max-w-[1100px] mx-auto" id="portafolio">
        <motion.div className="text-center mb-16 space-y-4"
          initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }}
          viewport={{ once:true, margin:"-80px" }} transition={{ duration:0.5, ease:"easeOut" }}>
          <p className="text-[11px] font-black tracking-[0.2em] uppercase" style={{ color:TEAL }}>Trabajo real</p>
          <h2 className="font-black text-balance"
            style={{ fontFamily:"var(--font-syne)", color:BONE, fontSize:"32px", lineHeight:"40px", letterSpacing:"-0.01em" }}>Proyectos que lanzamos</h2>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {projects.map((p, i) => (
            <motion.a key={p.title} href={p.href} target="_blank" rel="noopener noreferrer"
              ...
              <h4 className="font-black mb-1" style={{ fontFamily:"var(--font-syne)", color:BONE, fontSize:"24px", lineHeight:"32px" }}>{p.title}</h4>
              <p style={{ color:MUTED, fontSize:"15px" }}>{p.category}</p>
            </motion.a>
          ))}
        </div>
      </section>
