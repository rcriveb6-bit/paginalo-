    ...
    <div className="text-[#1E3A5F] overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center bg-[#F8F8F8] overflow-hidden px-6 text-center">
        ...
        <motion.div initial="hidden" animate="show" variants={stagger} className="relative z-10 max-w-3xl w-full">
          ...
          <motion.div variants={fadeUp} className="flex justify-center mb-10">
            <Image
              ...
              width={560}
              height={180}
              className="w-full max-w-[340px] sm:max-w-[440px] md:max-w-[520px] h-auto object-contain"
              priority
            />
          </motion.div>

          <motion.p variants={fadeUp} className="max-w-lg mx-auto text-base md:text-lg leading-relaxed text-[#1E3A5F]/55 mb-10">
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href={wa} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 rounded-full px-8 py-3.5 text-sm font-semibold text-white transition-all duration-200 hover:scale-[1.03]"
              style={{ backgroundColor: "#FF7F7F", boxShadow: "0 4px 20px rgba(255,127,127,0.3)" }}
            >
              <WaIcon /> Contáctanos gratis
            </a>
            <a href="#portafolio"
              className="inline-flex items-center justify-center gap-2 rounded-full border px-8 py-3.5 text-sm font-semibold transition-all duration-200 hover:bg-[#1E3A5F]/5"
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1.5">
          <span className="text-[10px] text-[#1E3A5F]/25 tracking-widest uppercase">Scroll</span>
          <div className="w-px h-8 bg-gradient-to-b from-[#00B5B5]/25 to-transparent" />
        </div>
      </section>
            ...
            {[
              {
                ...
                title: "Liora & Co.",
                description: "Una tienda online lista para recibir órdenes a cualquier hora. Automatizamos todo el proceso de compra para que Liora pueda vender sus velas a todo Puerto Rico.",
              ...
              },
              {
                ...
                title: "RYA Shop",
                description: "Una vitrina elegante que proyecta confianza. Inventario exclusivo conectado directamente a WhatsApp para cerrar ventas de forma rápida y personal.",
              ...
              const inner = (
                <div className={`group relative flex flex-col justify-between h-full rounded-2xl border border-gray-100 ${p.bg} p-7 shadow-sm transition-all duration-300 ${p.live ? "hover:shadow-md hover:border-[#00B5B5]/25 cursor-pointer" : "cursor-default"}`}>
                  <div className="absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl opacity-40 group-hover:opacity-100 transition-opacity"
                    style={{ background: `linear-gradient(90deg, ${p.accent}, transparent)` }} />
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-[10px] font-bold tracking-[0.18em] uppercase text-[#00B5B5]">{p.badge}</span>
                      <span className="text-[10px] font-medium text-[#1E3A5F]/30">{p.location}</span>
                    </div>
                    <h3 className={`text-xl font-bold text-[#1E3A5F] mb-3 ${p.live ? "group-hover:text-[#00B5B5]" : ""} transition-colors duration-300`}>
                      {p.title}
                      {!p.live && (
                        <span className="ml-2 text-[9px] font-semibold tracking-widest uppercase bg-[#00B5B5]/10 text-[#00B5B5] px-2 py-0.5 rounded-full align-middle">Próximamente</span>
                      )}
                    </h3>
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    <div className="flex flex-wrap gap-1.5">
                      {p.tags.map(t => (
                        <span key={t} className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#00B5B5]/10 text-[#00B5B5]">{t}</span>
                      ))}
                    </div>
                    {p.live && <span className="text-[#00B5B5] text-lg group-hover:translate-x-1.5 transition-transform duration-300 shrink-0 ml-2">→</span>}
                  </div>
                </div>
              );
              return (
                <motion.div key={p.title} variants={fadeUp} initial="hidden" whileInView="show"
                  viewport={{ once: true, margin: "-60px" }} transition={{ delay: p.delay }} className="h-full">
                  {p.href ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={s.title} variants={fadeUp} initial="hidden" whileInView="show"
                viewport={{ once: true, margin: "-60px" }} transition={{ delay: s.delay }}
              ...
              >
                <div className="mb-5 inline-flex items-center justify-center h-11 w-11 rounded-xl bg-[#00B5B5]/10 text-[#00B5B5] group-hover:bg-[#00B5B5]/15 transition-colors">
                  {s.icon}
                </div>
      ...
      <section id="contacto" className="relative overflow-hidden bg-[#1E3A5F]">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-96 h-48 rounded-full blur-3xl pointer-events-none"
          style={{ backgroundColor: "rgba(0,181,181,0.07)" }} />
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-28 text-center">
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }}>
            ...
            <a href={wa} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-3 rounded-full px-10 py-4 text-sm font-bold text-white transition-all duration-300 hover:scale-[1.04]"
